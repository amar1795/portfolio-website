          ...
          <div key={index} className="bg-black/50 border border-green-400/30 p-4 rounded-lg">
            <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-3">
              ...
              <div className="text-right mt-2 md:mt-0">
                <p className="text-green-200">{exp.location}</p>
                <p className="text-green-400 font-medium">{exp.duration}</p>
              </div>
            </div>
            <ul className="space-y-2 mt-3">
              {exp.responsibilities.map((responsibility, respIndex) => (
                <li key={respIndex} className="text-green-100 flex items-start">
